...
- **security-review**: the portfolio-wide vulnerability checklist + audit method. Tier 0 open doors (auth, IDOR, isolation, PostgREST filter injection, secrets, rate limits), Tier 1 "on but still leaks" (RLS policy holes incl. migrations-vs-live-state and role-scoped sensitive columns, bucket listing, pre-auth money pumps with per-payer caps and role-gating, SSRF, prompt injection / AI-takes-actions, trusting client-controlled request metadata like the Host header), and Tier 2 Feld scars (secrets tracked in git, reset-token-in-response, empty signing-secret fallback, plaintext provider keys, client-only PII redaction, fail-open tenant scoping, spoofable rate-limit keys, dead scaffolding). Reference checklist adds webhook claim-first idempotency and other money-path robustness. SSO-only removes the whole password class. Two-pass, verify-in-source, verify live infra state not just code, split code fixes from live-cred actions, independent-review gate. Pairs with the built-in `/security-review`.
...
- **in-house-observability**: error monitoring you own, no third-party service. Errors go to a Supabase `error_log` table (plus console); Sentry/webhook/OTel stay optional forwarders. Consistent cross-project schema, fingerprint grouping, PII-safe context, never-throws module, and a define/identify/fix triage workflow. Ships copy-paste `monitor.js` + `error_log.sql` and a graduation path to OpenTelemetry.
