...
- **security-review**: the portfolio-wide vulnerability checklist + audit method. Tier 0 open doors (auth, IDOR, isolation, secrets, rate limits), Tier 1 "on but still leaks" (RLS policy holes, bucket listing, pre-auth money pumps, SSRF, prompt injection / AI-takes-actions), and Tier 2 Feld scars (secrets tracked in git, reset-token-in-response, empty signing-secret fallback, plaintext provider keys, client-only PII redaction, dead scaffolding). SSO-only removes the whole password class. Two-pass, verify-in-source, split code fixes from live-cred actions, independent-review gate. Pairs with the built-in `/security-review`.
